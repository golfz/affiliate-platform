...
import { getAllCampaigns, createCampaign, createLink, deleteCampaign, getAllProducts, type CampaignResponse, type LinkResponse, type ProductResponse } from '@/lib/api'
...
export default function CampaignsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate links for each product and marketplace
      ...
      for (const productId of productIds) {
        for (const marketplace of ['lazada', 'shopee'] as const) {
          try {
            ...
            console.error(`Failed to create link for ${productId} - ${marketplace}:`, err)
          }
        }
      }
      setLinks(newLinks)
  ...
  return (
    <AdminLayout>
      <div className="px-4 sm:px-0">
        ...
        {campaign && (
          <div className="bg-white shadow rounded-lg p-6 mb-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900">Campaign Created</h2>
            <div className="space-y-2 text-gray-900">
              <p><strong>ID:</strong> {campaign.id}</p>
              <p><strong>Name:</strong> {campaign.name}</p>
              <p><strong>UTM Campaign:</strong> {campaign.utm_campaign}</p>
              <p><strong>Start:</strong> {new Date(campaign.start_at).toLocaleString()}</p>
              <p><strong>End:</strong> {new Date(campaign.end_at).toLocaleString()}</p>
            </div>
          </div>
        )}
        ...
        {links.length > 0 && (
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-xl font-semibold mb-4 text-gray-900">Generated Links</h2>
            <div className="space-y-4">
              {links.map((link) => (
                <div key={link.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-medium text-gray-900 capitalize">{link.marketplace}</p>
                      <p className="text-sm text-gray-600">Short Code: {link.short_code}</p>
                      <a
                        href={link.full_url}
                        target="_blank"
                        rel="noopener noreferrer"
                    ...
                    <button
                      onClick={() => navigator.clipboard.writeText(link.full_url)}
